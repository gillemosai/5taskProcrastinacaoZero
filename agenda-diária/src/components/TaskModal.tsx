...
interface TaskModalProps {
  ...
  selectedDate: string;
  initialTime?: string; // Predefined starting hour e.g. "09:00"
...
export default function TaskModal({
  ...
  // Sincronizar campos quando o modal abre ou muda de tarefa
  useEffect(() => {
    if (isOpen) {
      setError('');
      if (taskToEdit) {
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        >
          ...
          <div className="flex items-center justify-between border-b border-slate-100 px-6 py-4 dark:border-slate-800/80">
            ...
            <button
              onClick={onClose}
              className="rounded-lg p-1 text-slate-400 hover:bg-slate-50 hover:text-slate-600 dark:hover:bg-slate-800 dark:hover:text-slate-200 transition-colors"
            >
              <X className="h-5 w-5" />
            </button>
          </div>
          ...
          <form onSubmit={handleSubmit} className="p-6">
            {error && (
              <div className="mb-4 rounded-xl bg-red-50 p-3.5 text-xs font-medium text-red-600 dark:bg-red-950/20 dark:text-red-400 flex items-center gap-2">
            ...
            <div className="space-y-4">
              {/* Title Input */}
              <div>
                <label htmlFor="task-title" className="block text-xs font-semibold text-slate-500 uppercase tracking-wider dark:text-slate-400 mb-1.5">
                  Título da Tarefa
                </label>
                <input
                  id="task-title"
              ...
              {/* Descrição input */}
              <div>
                <label htmlFor="task-desc" className="block text-xs font-semibold text-slate-500 uppercase tracking-wider dark:text-slate-400 mb-1.5 flex items-center gap-1">
                ...
                <div className="flex flex-col justify-end">
                  <label className="flex items-center gap-2.5 rounded-xl border border-slate-200 bg-slate-50/20 px-4 py-3 cursor-pointer select-none hover:bg-slate-50 dark:border-slate-800 dark:bg-slate-950/40 dark:hover:bg-slate-950 transition-colors">
                    <input
                      ...
                      className="accent-indigo-600 rounded-sm h-4 w-4"
                    />
                        ...
                        <button
                          ...
                          className={`rounded-lg px-2.5 py-1 text-xs font-medium border transition-all ${
                            duration === preset.value
                              ...
                              : 'bg-slate-50 text-slate-600 border-slate-200 hover:bg-slate-100 hover:border-slate-300 dark:bg-slate-950 dark:text-slate-300 dark:border-slate-800 dark:hover:bg-slate-800 dark:hover:border-slate-700'
                          }`}
                        >
                          {preset.label}
                        </button>
                    ...
                    {Object.entries(CATEGORIES).map(([key, value]) => {
                      ...
                      return (
                        <button
                          ...
                          className={`flex items-center gap-1.5 rounded-xl border px-3 py-2 text-xs font-medium text-left transition-all ${
                            isSelected
                    ...
                    {Object.entries(PRIORITIES).map(([key, value]) => {
                      ...
                      return (
                        <button
                          ...
                          className={`flex items-center justify-between w-full rounded-xl border px-3 py-2 text-xs font-medium transition-all ${
                            isSelected
            ...
            <div className="mt-8 flex items-center justify-end gap-3 border-t border-slate-100 pt-5 dark:border-slate-800/80">
              <button
                type="button"
                onClick={onClose}
                className="rounded-xl border border-slate-200 bg-white px-5 py-2.5 text-sm font-semibold text-slate-600 hover:bg-slate-50 hover:text-slate-800 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-300 dark:hover:bg-slate-900 transition-colors"
              >
                Cancelar
              </button>
              <button
                type="submit"
                className="rounded-xl bg-indigo-600 px-6 py-2.5 text-sm font-semibold text-white shadow-md shadow-indigo-500/10 hover:bg-indigo-700 focus:ring-2 focus:ring-indigo-500/20 active:scale-[0.98] transition-all"
              >
