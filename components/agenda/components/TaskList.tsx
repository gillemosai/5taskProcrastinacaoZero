...
export default function TaskList({
  ...
  const handleQuickSubmit = (e: React.FormEvent) => {
    ...
    onAddTask(quickTitle.trim(), true);
    setQuickTitle('');
  };
        ...
        <div className="flex-1 flex flex-col p-5 overflow-hidden">
          ...
          <form onSubmit={handleQuickSubmit} className="mb-4">
            <div className="relative">
              ...
              <button
                type="submit"
                className="absolute right-2 top-1/2 -translate-y-1/2 rounded-lg p-1.5 bg-indigo-50 hover:bg-indigo-100 text-indigo-655 dark:bg-indigo-950/40 dark:text-indigo-455 transition-colors cursor-pointer"
                title="Adicionar"
              >
                ...
                return (
                  <div
                    ...
                    className={`group/item flex items-start gap-3 rounded-xl border p-3.5 transition-all ${
                      task.completed
                        ? 'bg-slate-50/50 border-slate-100 dark:bg-slate-955/20 dark:border-slate-850'
                        : 'bg-white border-slate-100 hover:border-slate-205 dark:bg-slate-900 dark:border-slate-800/70 hover:dark:border-slate-700'
                    }`}
                  >
                    {/* Checkbox */}
                    <button
                      onClick={() => onToggleComplete(task.id)}
                      className="mt-0.5 shrink-0 text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-850 rounded-md p-0.5 transition-colors focus:outline-hidden cursor-pointer"
                    ...
                    <div className="opacity-0 group-hover/item:opacity-100 flex items-center gap-0.5 ml-auto shrink-0 select-none transition-opacity duration-150">
                      <button
                        ...
                        className="rounded-lg p-1 hover:bg-slate-100 text-indigo-500 dark:hover:bg-slate-800 hover:text-indigo-650 cursor-pointer"
                        title="Editar"
                      >
                      ...
                      <button
                        ...
                        className="rounded-lg p-1 hover:bg-slate-100 text-red-500 dark:hover:bg-slate-800 hover:text-red-650 cursor-pointer"
                        title="Excluir"
                      >
            ...
            {pendingTasksAllTime.length === 0 ? (
              ...
              pendingTasksAllTime.map((task) => {
                ...
                return (
                  <div
                    key={task.id}
                    className="flex flex-col rounded-xl border border-slate-105 bg-white p-3.5 dark:bg-slate-900 dark:border-slate-800/70"
                  >
                    <div className="flex items-start justify-between gap-1 mb-2">
                      ...
                      <div className="flex items-center gap-1">
                        <button
                          onClick={() => onToggleComplete(task.id)}
                          className="p-1 rounded text-emerald-500 hover:bg-slate-50 dark:hover:bg-slate-850 cursor-pointer"
                          title="Marcar como Concluída"
                        >
                          <Square className="h-3.5 w-3.5" />
                        </button>
                        <button
                          onClick={() => onEditTask({ ...task, date: selectedDate })}
                          className="px-1.5 py-0.5 rounded text-[9px] font-bold text-indigo-650 bg-indigo-50 hover:bg-indigo-100 dark:text-indigo-400 dark:bg-indigo-950/20 cursor-pointer"
                    ...
                    {task.description && (
                      <p className="text-[10px] font-medium leading-normal mt-1 text-slate-400 opacity-80 break-words dark:text-slate-550 line-clamp-1">
                        {task.description}
                      </p>
                    )}
                    ...
                    <div className="mt-2.5 flex items-center justify-between border-t border-slate-50 dark:border-slate-850 pt-2 text-[9px] font-bold">
                      ...
                      <button
                        onClick={() => onDeleteTask(task.id)}
                        className="text-red-550 hover:scale-105 transition-transform cursor-pointer"
                      >
                        Excluir
                      </button>
                    </div>
